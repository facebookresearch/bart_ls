...
@register_task("long_denoising")
class LongDenoisingTask(LegacyFairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            default=False,
            action="store_true",
            help="Using a range of masked span lengths [4, 8, 16], to improve results on both QA and Summarization?"
        )
    ...
    def __init__(self, args, dictionary):
        ...
        # add sentinel tokens
        max_num_noise_tokens = int(np.round((args.max_source_positions - 2) * args.noise_density))
        max_num_noise_spans = int(np.round(max_num_noise_tokens / args.mean_noise_span_length))

        logger.info(f"Adding {max_num_noise_spans} new sentinel tokens")
        for sentinel_id in range(max_num_noise_spans):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = LongDenoisingDataset(
            ...
            seed=self.seed,
            noise_density=self.args.noise_density,
            ...
            dynamic_span=self.args.dynamic_span_len
        )
        logger.info(
